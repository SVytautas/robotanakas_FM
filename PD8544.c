...
void PCD8544_InitIO(void) {
  ...
  SpiPD8544.Instance               = PCD8544_SPI;
  SpiPD8544.Init.BaudRatePrescaler = SPI_BAUDRATEPRESCALER_32;
  SpiPD8544.Init.Direction         = SPI_DIRECTION_1LINE;
  SpiPD8544.Init.CLKPhase          = SPI_PHASE_2EDGE;
  SpiPD8544.Init.CLKPolarity       = SPI_POLARITY_HIGH;
  SpiPD8544.Init.CRCCalculation    = SPI_CRCCALCULATION_DISABLE;
  SpiPD8544.Init.CRCPolynomial     = 10;
  SpiPD8544.Init.DataSize          = SPI_DATASIZE_8BIT;
  SpiPD8544.Init.FirstBit          = SPI_FIRSTBIT_MSB;
  SpiPD8544.Init.NSS               = SPI_NSS_SOFT;
  SpiPD8544.Init.TIMode            = SPI_TIMODE_DISABLE;
  
//#ifdef MASTER_BOARD
  SpiPD8544.Init.Mode = SPI_MODE_MASTER;
//#else
 // SpiPD8544.Init.Mode = SPI_MODE_SLAVE;
//#endif /* MASTER_BOARD */

  if(HAL_SPI_Init(&SpiPD8544) != HAL_OK)
  {
...
uint8_t aTxBufferPD[10] = {0x00};
void PCD8544_send(unsigned char data) {
	PCD8544_CE_LOW;
	//TM_SPI_Send(PCD8544_SPI, data);
	aTxBufferPD[0]=data;
	HAL_SPI_Transmit(&SpiPD8544, (uint8_t*)aTxBufferPD, 1,0xFFFF);
	PCD8544_CE_HIGH;
}
...
void vprint_nokia(const  char *fmt, va_list argp)
{
    char string[256];
    if(0 < vsprintf(string,fmt,argp)) // 
    {
       // HAL_UART_Transmit(&Uartdebug, (uint8_t*)string, strlen(string), 0xffffff); // send message via UART
			uint16_t i=(uint16_t)strlen(string);
			uint8_t n=0;
			for ( n=0;n<i;n++)
			{PCD8544_Putc(string[n], PCD8544_Pixel_Set, PCD8544_FontSize_5x7);}
			
    }
}

void NOKIA_PRINTF(const char *fmt, ...)
{
    va_list argp;
    va_start(argp, fmt);
    vprint_nokia( fmt, argp);
    va_end(argp);
}
...
void PCD8544_fillbuff(char * buff)
{
	PCD8544_Home();
	memcpy(PCD8544_Buffer,buff,sizeof(PCD8544_Buffer));
	PCD8544_GotoXY(0, 0);
	PCD8544_UpdateArea(0, 0, PCD8544_WIDTH - 1, PCD8544_HEIGHT - 1);
